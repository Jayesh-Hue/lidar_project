...
kitti360Path = os.environ.get('KITTI360_DATASET', os.path.join(os.path.dirname(os.path.realpath(__file__)), '..', '..'))

seq, cam_id, frame = 0, 0, 13
...
folder_path = os.path.join("Results", f"scene_{frame}","Projected_pointcloud")
file_path = os.path.join(folder_path, f"{frame}.jpg")
...
seg_colors = {
    ...
    3: ((255, 0, 255), "Purple"),     # Purple((0, 255, 255), "Yellow"),     # Yellow (BGR)
    4: ((0, 165, 255), "Orange"),     # Orange
    5: ((0, 0, 0), "Black"),          # Black
    6: ((128, 128, 128), "Gray"),     # Gray
    7: ((42, 42, 165), "Brown"),      # Brown
    8: ((0, 255, 255), "Yellow"),  # Yellow (BGR)
    9: ((203, 192, 255), "Pink"),  # Pink
}
...
# Build the directory and file path
folder_path = os.path.join("Results", f"scene_{frame}","Projected_pointcloud_onlycars")
file_path = os.path.join(folder_path, f"{frame}.jpg")
...
# ==== FINAL VISUALIZATION WITH COLORED ORIGINAL POINT CLOUD ====

# Initialize colors for all points as gray
colors_full = np.tile(np.array([0.5, 0.5, 0.5]), (xyz_cam.shape[0], 1))  # Nx3 gray

# For each car, assign the car color to the corresponding points in the original point cloud
for car_id, pts in car_lidar_points.items():
    if pts.size == 0:
        continue

    # Find indices of these points in the original cloud (xyz_cam)
    # Since pts is a subset of xyz_cam, find matches:
    # We'll use a KDTree for efficient matching
    import scipy.spatial

    kdtree = scipy.spatial.cKDTree(xyz_cam[:, :3])
    _, indices = kdtree.query(pts[:, :3], k=1)

    bgr = seg_colors.get(car_id, ((1, 1, 1), "Unknown"))[0]
    rgb = [bgr[2] / 255, bgr[1] / 255, bgr[0] / 255]

    colors_full[indices] = rgb

# Create original point cloud with colored points
original_pcd = o3d.geometry.PointCloud()
original_pcd.points = o3d.utility.Vector3dVector(xyz_cam[:, :3])
original_pcd.colors = o3d.utility.Vector3dVector(colors_full)

vis_geometries = [original_pcd]

# Add segmented cars and boxes separately for clarity (optional)
for car_id, res in results_eval.items():
    bgr = seg_colors.get(car_id, ((1, 1, 1), "Unknown"))[0]
    rgb = [bgr[2] / 255, bgr[1] / 255, bgr[0] / 255]
